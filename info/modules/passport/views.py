...
from flask import abort, jsonify, current_app, session, make_response, request
...
def register():
    ...
    if real_sms_code != smscode:
        return jsonify(errno=RET.DATAERR, errmsg="验证码输入错误")
    # 很据用户输入的电话号码来查询数据库中是否存在，
    try:
        user = User.query.filter(User.mobile == mobile).first()
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="当前注册用户过多请稍后重试！")
    # 判断电话号码是否已被注册，
    if user:
        return jsonify(errno=RET.PARAMERR, errmsg="手机号已被注册！")
    else:
...
#
# # TODO 光标离开判断电话号码是否被注册
# @passport_blu.route('/phone_verification',methods=["POST"])
